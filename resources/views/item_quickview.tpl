<div class="qv_container">
    <div class="qv_left">
        ...
        <div class="qv_img">
            ...
            <img class="mausDrin" src="{$item.medien[0]->medium.bild_url_br}" {if $item.medien[0]->medium["beschreibung"] != ""}alt="{$item.medien[0]->medium["beschreibung"]}" title="{$item.medien[0]->medium["beschreibung"]}"{else}alt="{$item->name}" title="{$item->name}" {/if} id="img_zoom" data-zoom-image="{$item.medien[0]->medium.bild_url_gr}" />
            {else}
            ...
            <div class="">
                ...
                {/if}
                 <a class="itemName" href="{$item.itemLink}{if stristr($item.itemLink,'?from')}{-$addfrom}{elseif isset($addfrom)}?from={$addfrom}{/if}">
                   <b style="color:#a58c4a!important;"> {$item->name} </b>
                 </a>
