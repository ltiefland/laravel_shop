                ...
                <div class="item_slider_inner">
                    ...
                    <div class="item_slider_img">
                        <a href="{$item.itemLink}" title="Zum Artikel {$item.kurzbezeichnung}">
                            ...
                            <img style="max-height:120px;" src="{$item.medien[0]->medium.bild_url_mini}" {if $item.medien[0]->medium["beschreibung"] != ""}alt="{$item.medien[0]->medium["beschreibung"]}" title="{$item.medien[0]->medium["beschreibung"]}"{else}alt="{$item.kurzbezeichnung}" title="{$item.kurzbezeichnung}" {/if}/>
                            {/if}
                        </a>
                    </div>
                    <div class="item_slider_wrapper">
                        <h3 class="bezeichnung uppercase">
                            <a href="{$item.itemLink}" title="Zum Artikel {$item.kurzbezeichnung}">{$item.kurzbezeichnung|truncate:"45"}</a>
                        </h3>
