...
<div class="addonItems_click">
    ...
    <div class="box_addons" id="a_{$item.ID}">
        <a style="cursor:pointer;">
            <img src="{$item.medien[0]->medium.bild_url_mini}" width="50" {if $item.medien[0]->medium["beschreibung"] != ""}alt="{$item.medien[0]->medium["beschreibung"]}" title="{$item.medien[0]->medium["beschreibung"]}"{else}alt="{$item->name}" title="{$item->name}" {/if} />
        </a>
    </div>
    <!-- ENDE Artikel -->
        ...
        <div>
            <div style="width:100%;display:table-cell;vertical-align:middle;text-align:center;height:125px;">
                <a href="{$item.itemLink}">
                    <img src="{$item.medien[0]->medium.bild_url_mini|urldecode}" style="max-width:125px;max-height:120px;" {if $item.medien[0]->medium["beschreibung"] != ""}alt="{$item.medien[0]->medium["beschreibung"]}" title="{$item.medien[0]->medium["beschreibung"]}"{else}alt="{$item->name}" title="{$item->name}" {/if} />
                </a>
            </div>
            <div style="text-align:center;">
                <div class="small" style="">
                <a href="{$item.itemLink}">{$item->name|truncate:100:"...":true:false}</a>
                </div>
